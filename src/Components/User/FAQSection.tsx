...
const FAQSection: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const model = genAI.getGenerativeModel({ model: "gemini-pro" });
      const prompt = `You are an AI assistant for a luxury car service company. Only answer questions related to car servicing, maintenance, and automotive topics. If the user asks about anything else, politely remind them to ask only about car service-related topics. 
      ...
      <AnimatePresence>
        {isChatOpen && (
          ...
          >
            <Card className="h-full flex flex-col">
              <div className="p-4 border-b flex justify-between items-center">
                <h3 className="text-xl font-bold">Chat with Quickfix</h3>
                <Button variant="ghost" onClick={() => setIsChatOpen(false)}>
                  <X size={24} />
                </Button>
              </div>
